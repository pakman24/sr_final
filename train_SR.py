...
# 초해상도 모델 #$$
#out_highv = model_FSRCNN( in_lowv);
out_highv = model_DRRN( in_lowv, op_rs);
...
sr_vars = t_vars;
#sr_vars = [var for var in t_vars if 'model_FSRCNN' in var.name]; #  나중에 쓸모있는 코드라 남겨둠

# 트레이닝 스탭
train_sr = tf.train.AdamOptimizer(TSET.learning_rate).minimize(loss_L1,var_list=sr_vars);
...
#    print('[%d]'%(epoch),end='')
    ...
    for i in range(num_batch):
        ...
        # 배치 학습상황 출력
        print('\r batch: %4d/%d  qu: %d'%(i,num_batch,dset_train.qu_data.qsize()),end='');
                    
        # 텐서보드
        image_num = TSET.size_batch * (num_batch * epoch+i);
        summary_writer_train.add_summary(summary_train, image_num);
        
        #######
        # validation 은 10번마다 한번씩 수행
        if( i%10== 0 ):
            ...
            [summary_valid] = sess.run([merged],
                feed_dict={ in_lowv      : batch_vld.arr4d_LR_v,
                            ...
                            op_rs        : rs_factor} );
            summary_writer_valid.add_summary(summary_valid, image_num);
